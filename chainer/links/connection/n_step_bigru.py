...
from chainer.functions.connection.n_step_bigru import n_step_bigru
...
class NStepBiGRU(link.ChainList):
    ...
    def __call__(self, hx, xs, train=True):
        ...
        indices = argsort_list_descent(xs)  # MEMO: xs is sorted here!!
        ...
        hy, trans_y = n_step_bigru(
            self.n_layers, self.dropout, hx, ws, bs, trans_x,
            train=train, use_cudnn=self.use_cudnn)

        hy = permutate.permutate(hy, indices, axis=1, inv=True)  # MEMO: inverse sort here
